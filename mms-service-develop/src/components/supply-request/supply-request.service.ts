...
import { DeviceReturnTicketStatus } from 'src/models/device-return-ticket/device-return-ticket.schema';
...
export class SupplyRequestService implements SupplyRequestServiceInterface {
  ...
  async listImo(
    ...
    const deviceReturns =
      await this.deviceReturnTicketRepository.findAllWithPopulate(
        {
          status: DeviceReturnTicketStatus.AwaitingReturn,
        },
        {
    ...
    deviceReturns.forEach((e) => {
      items.push({
        ...
        planTo: moment(e.createdAt).add(1, 'day').toDate(),
        type: 1,
        items: e.deviceAssignments.map((v) => ({
          id: v['deviceId']._id,
          code: v['deviceId'].code,
          planQuantity: 1,
          type: 1,
          unit: 1,
        })),
  ...
  async detailImo(
    ...
    if (!data.length) {
      data = await this.deviceReturnTicketRepository.findAllWithPopulate(
        {
          status: DeviceReturnTicketStatus.AwaitingReturn,
          _id: request.id,
        },
  ...
  async updateActualQuantity(
    ...
    if (
      ...
      itemMap.get(deviceRequest.device.toString())
    ) {
      deviceRequest.actualImportQuantity = plus(
        deviceRequest.actualImportQuantity,
        itemMap.get(deviceRequest.device.toString()),
      );
      if (deviceRequest.actualImportQuantity > deviceRequest.quantity) {
        return new ApiError(
          ResponseCodeEnum.BAD_REQUEST,
