  ...
  implements DeviceAssignmentServiceInterface
{
  ...
  async createAssignDevice(
    ...
    if (deviceRequest.status !== DeviceRequestTicketStatus.AwaitingAssignment) {
      return new ApiError(
        ResponseCodeEnum.BAD_REQUEST,
    ...
    const countDeviceAssign =
      await this.deviceAssignmentRepository.countDocumentsByDeviceRequest(
        request.deviceRequestId,
      );

    if (deviceRequest.actualImportQuantity <= countDeviceAssign) {
      return new ApiError(
        ResponseCodeEnum.BAD_REQUEST,
  ...
  async updateAssignDevice(
    ...
    if (
      ...
      if (
        oldDeviceRequest.status === DeviceRequestTicketStatus.Confirmed &&
    ...
    if (
      deviceRequest.status === DeviceRequestTicketStatus.Confirmed &&
      deviceAssignmentExists.status === DeviceAssignStatus.Confirmed
    ) {
      return new ApiError(
        ResponseCodeEnum.BAD_REQUEST,
  ...
  async getAssignDevice(
    ...
    const factory = await this.userService.detailFactoryById(
  ...
  private async updateStatusRequestDevice(
    ...
    if (minus(deviceRequest.quantity || 0, 1) === deviceAssign.length) {
      deviceRequest.status = DeviceRequestTicketStatus.Assigned;
      await deviceRequest.save();
    }
    ...
    if (
      ...
      if (oldDeviceRequest.status === DeviceRequestTicketStatus.Assigned) {
        oldDeviceRequest.status = DeviceRequestTicketStatus.AwaitingAssignment;
