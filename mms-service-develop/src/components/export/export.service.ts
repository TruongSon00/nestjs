...
import { GetListAttributeTypeQuery } from '@components/attribute-type/dto/query/get-list-attribute-type.query';
...
import { GetListItemUnitSettingResponseDto } from './../item/dto/response/get-list-item-unit-setting.response.dto';
  ...
  async exportRegion(payload: ExportRequestDto) {
    ...
    const headers = [
      ...
      },
      {
        ...
        width: 15,
        style: { alignment: <Partial<Alignment>>EXCEL_STYLE.ALIGN_LEFT_MIDDLE },
        title: await this.i18n.translate('export.region.interRegionCode'),
      },
    ];
    ...
    const items = regions?.map((region) => {
      const item = {
        ...
        interRegionCode: region?.interRegion?.code ?? '',
      };
