...
import { ImportDeviceResultDto } from '@components/device/dto/response/import-device-result.dto';
import { ImportRequestDto } from '@components/import/dto/request/import.request.dto';
import { ImportDeviceRowRequestDto } from '@components/device/dto/request/import-device-row.request.dto';
import {
...
import { AssignTypeEnum } from '../../models/device-assignment/device-assignment.schema';
import {
...
export class DeviceService implements DeviceServiceInterface {
  ...
  async scanQrCode(request: ScanQrDeviceRequestDto): Promise<any> {
    ...
    try {
      ...
      // TODO: check user permission by responsibleUser instead of device assignment's user
      // if (response.responsibleUserId !== userId) {
      //   return new ResponseBuilder()
      //     .withCode(ResponseCodeEnum.BAD_REQUEST)
      //     .withMessage(
      //       await this.i18n.translate('error.DEVICE_PERMISSION_DENIED'),
      //     )
      //     .build();
      // }
      const deviceRequest =
      ...
      const response = await this.deviceRepository.findOneBySerial(
  ...
  async getMaintenanceInfoApp(
    ...
    const device = await this.deviceRepository.findOneBySerial(
      deviceRequest.device,
    );
    if (!device) {
      return new ResponseBuilder()
  ...
  async exportDevice(request: ExportDeviceRequestDto): Promise<any> {
    ...
    for (const e of devices) {
      const deviceGroup = await this.deviceGroupRepository.detail(
        e.deviceGroup,
      );
  ...
  async import(
  ...
  ): Promise<ResponsePayload<ImportResponseDto<ImportDeviceResultDto>>> {
    const worksheet = await this.importService.loadXlsx(request);

    const { COLUMNS, ENTITY_KEY, REQUIRED_FIELDS } = IMPORT_DEVICE_CONST;
    ...
    const response = new ImportResponseDto<ImportDeviceResultDto>(
      worksheet.actualRowCount - HEADER_ROW_INDEX,
    );

    const columnMap = validateColumnsResponse.data as Map<string, string>;

    const isForManufacturingBoolean = await this.i18n.translate(
      'header-csv.boolean.true',
    );

    for (let i = DATA_ROW_START; i <= worksheet.actualRowCount; i++) {
      const row = worksheet.getRow(i);
      const action = getCellValueByRow(row, 1);

      if (isEmpty(action)) continue;

      const rowDto = new ImportDeviceRowRequestDto();

      rowDto.action = action;
      this.importService.assignProperty(COLUMNS, row, rowDto);

      const result = plainToInstance(ImportDeviceResultDto, rowDto, {
        excludeExtraneousValues: true,
      });
      ...
      if (!result.isSuccess) response.results.push(result);
        ...
        let entityDto: CreateDeviceRequestDto | UpdateDeviceRequestDto;

        entityDto = plainToInstance(CreateDeviceRequestDto, rowDto, {
          excludeExtraneousValues: true,
        });

        entityDto.userId = request.historyUserId;
        ...
        if (isForManufacturing == isForManufacturingBoolean)
          entityDto.type = DeviceType.ForManufacture;
        else entityDto.type = DeviceType.Normal;

        const maintenanceAttribute = (
          await this.maintenanceAttributeService.findOneByCondition({
            name: new RegExp(`^${maintenanceAttributeName}$`, 'i'),
          })
        ).data;

        if (maintenanceAttribute)
          entityDto.maintenanceAttributeId = maintenanceAttribute.id;
        ...
        if (!isEmpty(responsibleUser)) {
          ...
          if (user) {
            entityDto.responsibleSubject = {
              id: user.id,
              type: ResponsibleSubjectType.User,
            };
          }
        } else if (!isEmpty(responsibleMaintenanceTeam)) {
          const maintenanceTeam = (
            await this.maintenanceTeamService.findOneByCondition({
              name: responsibleMaintenanceTeam,
            })
          ).data;
        ...
        switch (action) {
          case availableActions.get(CREATE):
            addOrUpdateResponse = await this.create(entityDto);
            break;
          case availableActions.get(UPDATE):
            const detailDeviceResponse = await this.findOneByCode(
              entityDto.code,
            );

            if (detailDeviceResponse.statusCode != ResponseCodeEnum.SUCCESS)
              addOrUpdateResponse = detailDeviceResponse;
            else {
              entityDto = plainToInstance(UpdateDeviceRequestDto, rowDto, {
                excludeExtraneousValues: true,
              });
              addOrUpdateResponse = await this.update(
                entityDto as UpdateDeviceRequestDto,
              );
            }
            break;
        }
    ...
    return new ResponseBuilder<ImportResponseDto<ImportDeviceResultDto>>()
      .withCode(ResponseCodeEnum.SUCCESS)
      .withData(response)
      .build();
  }
  ...
  > {
    ...
    return new ResponseBuilder<GetImportTemplateResponseDto>()
      .withCode(ResponseCodeEnum.SUCCESS)
      .withData({
        fileName: IMPORT_DEVICE_CONST.FILE_NAME,
